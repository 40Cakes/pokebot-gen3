...
class Emulator:
    ...
    def __symbols(self):
        if self.sym_file:
            ...
            if self.game_code[3] in ['D', 'I', 'S', 'F', 'J'] and \
                    os.path.exists('modules/data/symbols/patches/language/{}'.format(
                        self.sym_file.replace('.sym', '.json'))):
                lang_patches = json.loads(ReadFile('modules/data/symbols/patches/language/{}'.format(
                    self.sym_file.replace('.sym', '.json'))))
...
def GetTaskFunc(func: str) -> TaskFunc:
    ...
    match func:
        case 'HANDLEDEFAULTPARTYMENU' | "HANDLEPARTYMENUSWITCHPOKEMONINPUT" | "HANDLEBATTLEPARTYMENU":
