...
class KeyboardNavigator(BaseMenuNavigator):
    ...
    def wait_for_keyboard(self):
        while (self.keyboard.cur_pos[0] > self.w and self.keyboard.cur_pos[1] > self.h) or len(self.keyboard.text_buffer) > 0:
                ...
                if spot == last_pos or (last_pos is None and spot[0] <= self.w and spot[1] <= self.h):
                    if page != goto[2]: # Press Select until on correct page
                        ...
                        if spot[0] < goto[0]:
                            ...
                            press = "Left"
                        elif (spot[1] < goto[1] or (spot[1] == self.h - 1 and goto[1] == 0)) and not (spot[1] == 0 and goto[1] == self.h-1):
                            press = "Down"
                        else:
                            press = "Up"
    ...
    def confirm_name(self):
        while self.keyboard.enabled:
            if self.keyboard.cur_pos[0] > self.w or (self.keyboard.cur_pos == (8,0) and context.rom.game_title in ["POKEMON RUBY", "POKEMON SAPP"]):
                context.emulator.press_button("A")
                yield
            else:
