...
WEBHOOK_DEFAULTS = {'consequent_interval': 5000, 'enable': False, 'first_interval': 8192, 'interval': 5, 'ping_id': None, 'ping_mode': None}
DEFAULT_CONFIG = {
    ...
    'discord': {
        ...
        'pokemon_encounter_milestones': {'consequent_interval': 5000, 'enable': False, 'first_interval': 8192, 'interval': 10000, 'ping_id': None, 'ping_mode': None},
        'rich_presence': False,
        ...
        'total_encounter_milestones': {'consequent_interval': 5000, 'enable': False, 'first_interval': 8192, 'interval': 25000, 'ping_id': None, 'ping_mode': None},
    },
    'general': {'starter': config.schemas_v1.Starters.MUDKIP},
    'keys': {
        'gba': {'Up': 'Up', 'Down': 'Down', 'Left': 'Left', 'Right': 'Right', 'A': 'x', 'B': 'z', 'L': 'a', 'R': 's', 'Start': 'Return', 'Select': 'BackSpace'},
        'emulator': {'zoom_in': 'plus', 'zoom_out': 'minus', 'toggle_manual': 'Tab', 'toggle_video': 'v', 'toggle_audio': 'b', 'set_speed_1x': '1', 'set_speed_2x': '2', 'set_speed_3x': '3', 'set_speed_4x': '4', 'set_speed_unthrottled': '0', 'reload_config': 'Ctrl+C', 'reset': 'Ctrl+R', 'exit': 'Ctrl+Q', 'save_state': 'Ctrl+S', 'toggle_stepping_mode': 'Ctrl+L'}},
    'logging': {
        'console': {'encounter_data': 'verbose', 'encounter_ivs': 'verbose', 'encounter_moves': 'disable', 'statistics': 'verbose'},
        'import_pk3': False,
...
CONFIG_TESTS = {
    'config_load': {
        'defaults load correctly': {
            'kwargs': {'config_dir': Path('test')},
            'expected': DEFAULT_CONFIG
        },
        'folder loads correctly': {
            'kwargs': {'config_dir': Path('test') / 'config'},
            'expected': DEFAULT_CONFIG.copy() | {'cheats': {'starters': True, 'starters_rng': True}}
        },
        'profile loads correctly': {
            'kwargs': {'config_dir': (Path('test') / 'config') / 'profile', 'is_profile': True},
            'expected': DEFAULT_CONFIG.copy() | {'metadata': {
                'rom': {'file_name': 'Pokemon - Emerald Version (U).gba', 'game_code': 'BPE', 'language': 'E', 'revision': 0},
                'version': 1}
            }
        },
        'is_profile but missing metadata': {
            'kwargs': {'config_dir': Path('test') / 'config', 'is_profile': True},
            'raises': exceptions.CriticalFileMissing
        },
        'missing file with strict = True': {
            'kwargs': {'config_dir': Path('test') / 'config', 'strict': True},
            'raises': exceptions.CriticalFileMissing
        }
    }
}
...
@pytest.mark.parametrize('test', CONFIG_TESTS['config_load'].values(), ids=CONFIG_TESTS['config_load'].keys())
