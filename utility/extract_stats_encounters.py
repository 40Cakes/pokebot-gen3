...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="append",
        dest="bot_mode",
        help="Only export encounters generated by a particular bot mode. This can be used multiple times.",
    )
    parser.add_argument(
        "--outcome",
        action="append",
    ...
    if not profile_directory.exists() or not profile_directory.is_dir():
        print(f"Profile `{args.profile}` does not exist.", file=sys.stderr)
        sys.exit(1)
        ...
        for map_name in args.map:
            if re.match(r"^\d+:\d+$", map_name):
                ...
                if map_group_and_number not in map_enum:
                    print(
                        f"Map `{map_name}` could not be found.",
                        file=sys.stderr,
                    )
                else:
    ...
    bot_mode_names = list(map(lambda s: s.lower(), get_bot_mode_names()))
    if args.bot_mode is not None:
        bot_mode_where_clause = []

        for bot_mode in args.bot_mode:
            if bot_mode.lower() not in bot_mode_names:
                print(
                    f"Bot mode `{bot_mode}` does not exist.",
                    file=sys.stderr,
                )
            else:
                for bot_mode_correct_case in get_bot_mode_names():
                    if bot_mode_correct_case.lower() == bot_mode.lower():
                        bot_mode_where_clause.append(f"bot_mode = ?")
                        where_parameters.append(bot_mode_correct_case)
    ...
    if args.from_date is not None:
        if re.match(r"^[1-9]\d+$", args.from_date):
            ...
            print(
                f"`from` value is neither an encounter ID nor a valid date format: `{args.from_date}`.",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if args.to_date is not None:
        if re.match(r"^[1-9]\d+$", args.to_date):
            ...
            print(
                f"`to` value is neither an encounter ID nor a valid date format: `{args.to_date}`.",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    result = stats_db._query_encounters(
